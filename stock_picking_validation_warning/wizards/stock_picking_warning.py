...
class StockImmediateTransfer(models.TransientModel):
    ...
    def _compute_should_block(self):
        for rec in self:
            ...
            rec.should_block = partner_id.picking_warn == 'block' and partner_id.picking_warn_msg
    ...
    def process(self):
        if self.should_block:
            raise UserError(_("You can not process the picking because the partner has set the option to block the picking."))

        return self.with_context(skip_picking_warning_check=True).picking_id.button_validate()
